...
public abstract class ReusableLibrary {
     ...
     * properties file
     */
    protected Properties objectPropertics;
    ...
    public ReusableLibrary(ScriptHelper scriptHelper) {
        ...
        this.test = scriptHelper.getReportTest();
        ...
        objectPropertics = Settings.getObjectPropertiesInstance();
        objectMap = new ObjectMap();

        //Web and Mobile web configuration
        ...
        if (strExecutionMode.equalsIgnoreCase("LOCAL") ||
                strExecutionMode.equalsIgnoreCase("REMOTE")) {
                deviceType = "Web";
        }
        else if (strExecutionMode.equalsIgnoreCase("LOCAL_EMULATED_DEVICE") || strExecutionMode.equalsIgnoreCase("REMOTE_EMULATED_DEVICE")) {
            String strTestingPlatform = scriptHelper.getTestParameters().getTestingPlatform().toString();
                if(strTestingPlatform.equalsIgnoreCase("TABLET_BROWSER")){
                    deviceType = "TabletWeb";
                }else if(strTestingPlatform.equalsIgnoreCase("MOBILE_BROWSER")){
                    deviceType = "MobileWeb";
                }else{
                report.updateTestLog("TestingPlatform","Please provide valid execution Testing platform in Configuration sheet column 'TestingPlatform, eg.TABLET_BROWSER,TABLET_APP,MOBILE_BROWSER,MOBILE_APP",Status_CRAFT.FAIL);
                return;
            }
        }
        else if(strExecutionMode.equalsIgnoreCase("MOBILE")|| strExecutionMode.equalsIgnoreCase("MOBILECENTRE")){
            String strTestingPlatform = scriptHelper.getTestParameters().getTestingPlatform().toString();
                if(strTestingPlatform.equalsIgnoreCase("TABLET_APP")||strTestingPlatform.equalsIgnoreCase("MOBILE_APP")){
                    isMobile = true;

                    if(strTestingPlatform.equalsIgnoreCase("TABLET_APP")){
                        deviceType = "TabletWeb";
                    }else{
                        deviceType = "MobileWeb";
                    }

                }else if(strTestingPlatform.equalsIgnoreCase("TABLET_BROWSER")){
                    deviceType = "TabletWeb";
                }else if(strTestingPlatform.equalsIgnoreCase("MOBILE_BROWSER")){
                    deviceType = "MobileWeb";
                }else{
            ...
            if((isMobile) &&(scriptHelper.getTestParameters().getMobileExecutionPlatform().toString().equalsIgnoreCase("IOS"))){
                isIOS = true;
            }
            ...
            switch (responseCode) {
				 ...
				 */
                    report.updateTestLog(linkValue, "Response code : "
                            + responseStatus + " - OK", Status_CRAFT.PASS);
                    break;
                case 3:
				 ...
				 */
                    report.updateTestLog(linkValue, "Response code : "
                            + responseStatus + " - Client error", Status_CRAFT.FAIL);
                    break;
    ...
    /**
     * Function to read object properties from object repo files
     *
     * @param objKey
     */
    protected By getObject(String objKey) {

        return objectMap.getObjectBy(objKey, objectPropertics);
    }

    /**
     * Function to return stored object property
     */
    protected String getObjectStoredProperty(String objKey) {
        return objectPropertics.getProperty(objKey);
    }
    ...
    protected String deviceType() {
        ...
        try {
            ...
            throw new FrameworkException("Exception in Method deviceType(): " + e);
        }
    }
