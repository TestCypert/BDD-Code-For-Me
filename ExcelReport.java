...
class ExcelReport implements ReportType {
	...
	public ExcelReport(ReportSettings reportSettings, ReportTheme reportTheme) {
		...
		testLogAccess = new ExcelDataAccess(reportSettings.getReportPath()
				+ Util.getFileSeparator() + "Excel Results",
				reportSettings.getReportName());

		resultSummaryAccess = new ExcelDataAccess(
				reportSettings.getReportPath() + Util.getFileSeparator()
	...
	private void initializeTestLogColorPalette() {
		testLogAccess.setCustomPaletteColor((short) 0x8,
				reportTheme.getContentBackColor());
		testLogAccess.setCustomPaletteColor((short) 0x9,
				reportTheme.getContentForeColor());
		testLogAccess.setCustomPaletteColor((short) 0xA,
				reportTheme.getsubHeadingBackColor());
		testLogAccess.setCustomPaletteColor((short) 0xB,
				reportTheme.getContentForeColor());
		testLogAccess.setCustomPaletteColor((short) 0xC,
				reportTheme.getSectionBackColor());
		testLogAccess.setCustomPaletteColor((short) 0xD,
	...
	public void updateTestLog(String stepNumber, String stepName,
			String stepDescription, Status_CRAFT stepStatus, String screenShotName) {
		testLogAccess.setDatasheetName(TEST_LOG);
		int rowNum = testLogAccess.addRow();
		...
		testLogAccess.setValue(rowNum, columnNum, stepStatus.toString(),
				cellFormatting);
		...
		testLogAccess.setValue(rowNum, "Step_Time", Util
				.getCurrentFormattedTime(reportSettings.getDateFormatString()),
				cellFormatting);
		...
		if (stepContainsScreenshot) {
			if (reportSettings.shouldLinkScreenshotsToTestLog()) {
				testLogAccess.setHyperlink(rowNum, columnNum,
						"..\\Screenshots\\" + screenShotName);

				testLogAccess.setValue(rowNum, "Description", stepDescription,
						cellFormatting);
			} else {
				testLogAccess.setValue(rowNum, "Description", stepDescription
	...
	private boolean processStatusColumn(Status_CRAFT stepStatus) {
		boolean stepContainsScreenshot = false;

		switch (stepStatus) {
		case PASS:
			cellFormatting.setForeColorIndex((short) 0xE);
			stepContainsScreenshot = reportSettings
	...
	public void addTestLogFooter(String executionTime, int nStepsPassed,
		...
		testLogAccess.setValue(rowNum, 0, "Execution Duration: "
				+ executionTime, cellFormatting);
		testLogAccess.mergeCells(rowNum, rowNum, 0, 4);
		...
		cellFormatting.setForeColorIndex((short) 0xE);
		testLogAccess.setValue(rowNum, "Step_No", "Steps passed",
				cellFormatting);
		testLogAccess.setValue(rowNum, "Step_Name", ": " + nStepsPassed,
				cellFormatting);
		cellFormatting.setForeColorIndex((short) 0x8);
		testLogAccess.setValue(rowNum, "Description", "", cellFormatting);
		cellFormatting.setForeColorIndex((short) 0xF);
		testLogAccess
				.setValue(rowNum, "Status", "Steps failed", cellFormatting);
		testLogAccess.setValue(rowNum, "Step_Time", ": " + nStepsFailed,
				cellFormatting);
	...
	private void initializeResultSummaryColorPalette() {
		resultSummaryAccess.setCustomPaletteColor((short) 0x8,
				reportTheme.getHeadingBackColor());
		resultSummaryAccess.setCustomPaletteColor((short) 0x9,
				reportTheme.getHeadingForeColor());
		resultSummaryAccess.setCustomPaletteColor((short) 0xA,
				reportTheme.getSectionBackColor());
		resultSummaryAccess.setCustomPaletteColor((short) 0xB,
				reportTheme.getSectionForeColor());
		resultSummaryAccess.setCustomPaletteColor((short) 0xC,
				reportTheme.getContentBackColor());
		resultSummaryAccess.setCustomPaletteColor((short) 0xD,
				reportTheme.getContentForeColor());
	...
	public void updateResultSummary(TestParameters testParameters,
		...
		cellFormatting.centred = false;
		cellFormatting.bold = false;
		resultSummaryAccess.setValue(rowNum, TEST_SCENARIO, scenarioName,
				cellFormatting);
		resultSummaryAccess.setValue(rowNum, TEST_CASE, testcaseName,
				cellFormatting);

		int columnNum = resultSummaryAccess.getColumnNum(TEST_INSTANCE, 0);
		resultSummaryAccess.setValue(rowNum, columnNum, testInstanceName,
				cellFormatting);
		if (reportSettings.shouldLinkTestLogsToSummary()) {
			resultSummaryAccess.setHyperlink(rowNum, columnNum, testReportName
					+ ".xls");
		}

		resultSummaryAccess.setValue(rowNum, TEST_DESCRIPTION,
				testcaseDescription, cellFormatting);
		resultSummaryAccess.setValue(rowNum, ADDITIONAL_DETAILS,
				additionalDetails, cellFormatting);

		cellFormatting.centred = true;
		resultSummaryAccess.setValue(rowNum, EXECUTION_TIME, executionTime,
				cellFormatting);
		...
		resultSummaryAccess.setValue(rowNum, TEST_STATUS, testStatus,
				cellFormatting);
	}
	...
	public void addResultSummaryFooter(String totalExecutionTime,
		...
		cellFormatting.setForeColorIndex((short) 0xE);
		resultSummaryAccess.setValue(rowNum, TEST_SCENARIO, "Tests passed",
				cellFormatting);
		resultSummaryAccess.setValue(rowNum, TEST_CASE, ": " + nTestsPassed,
				cellFormatting);
		cellFormatting.setForeColorIndex((short) 0x8);
