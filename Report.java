...
public class Report {
	...
	public Report(ReportSettings reportSettings, ReportTheme reportTheme, SeleniumTestParameters testParameters, ExtentTest test) {
	...
	public void initialize() {
		...
		//Commenting since perfecto and see test report not required
	 ...
	 */
	public void updateTestLog(String stepName, String stepDescription, Status_CRAFT stepStatus) {
		...
		if (stepStatus.ordinal() <= reportSettings.getLogLevel()) {
			...
			for (int i = 0; i < reportTypes.size(); i++) {
				...
				if (Boolean.parseBoolean(properties.getProperty("UpdateInJira"))) {
					if (stepStatus == Status_CRAFT.FAIL)
						System.out.println("Commented below lines");
			...
			//Screenshot path for extent report
				String screenshotpath = reportSettings.getReportPath() + Util.getFileSeparator() + SCREENSHOTS
						+ Util.getFileSeparator()
						+ screenshotName;
                strScreenshotpath = screenshotpath;
			try{
                if(stepStatus == Status_CRAFT.PASS) {
				...
				}else if (stepStatus == Status_CRAFT.FAIL) {
				...
				} else if(stepStatus == Status_CRAFT.WARNING) {
					test.log(Status.WARNING, MarkupHelper.createLabel(stepDescription, ExtentColor.ORANGE));
					test.warning("", MediaEntityBuilder.createScreenCaptureFromPath(screenshotpath).build());

				} else if(stepStatus == Status_CRAFT.DONE) {
                    test.log(Status.INFO, MarkupHelper.createLabel( stepDescription, ExtentColor.GREEN));

                } else if(stepStatus == Status_CRAFT.DEBUG) {
					test.log(Status.DEBUG, MarkupHelper.createLabel(stepDescription, ExtentColor.AMBER));
					test.info("", MediaEntityBuilder.createScreenCaptureFromPath(screenshotpath).build());

				} else if(stepStatus == Status_CRAFT.SCREENSHOT) {
					test.log(Status.INFO, MarkupHelper.createLabel(stepDescription, ExtentColor.BLUE));
					test.info("", MediaEntityBuilder.createScreenCaptureFromPath(screenshotpath).build());

				} else if(stepStatus == Status_CRAFT.FULLPAGESCREENSHOT) {
					test.log(Status.INFO, MarkupHelper.createLabel(stepDescription, ExtentColor.CYAN));
					test.info("", MediaEntityBuilder.createScreenCaptureFromPath(screenshotpath).build());
	...
	private void handleStepInvolvingPassOrFail(String stepDescription, Status_CRAFT stepStatus) {
		if (stepStatus.equals(Status_CRAFT.FAIL)) {
			testStatus = "Failed";
			...
			nStepsFailed++;
		} else if (stepStatus.equals(Status_CRAFT.PASS)) {
			nStepsPassed++;
		}
	}

	private String handleStepInvolvingScreenshot(String stepName, Status_CRAFT stepStatus) {
		String screenshotName = reportSettings.getReportName() + "_"
				...
				+ "_" + stepName.replaceAll("[^a-zA-Z0-9]+", "") + ".png";

		if ((stepStatus.equals(Status_CRAFT.FAIL) && reportSettings.shouldTakeScreenshotFailedStep())
				|| (stepStatus.equals(Status_CRAFT.PASS) && reportSettings.shouldTakeScreenshotPassedStep())
				|| stepStatus.equals(Status_CRAFT.SCREENSHOT)|| stepStatus.equals(Status_CRAFT.FULLPAGESCREENSHOT)) {

			String screenshotPath = reportSettings.getReportPath() + Util.getFileSeparator() + SCREENSHOTS
					+ Util.getFileSeparator() + screenshotName;

			String screenshotPath_Extent = reportSettings.getReportPath() + Util.getFileSeparator() + "Screenshots_Extent"
					+ Util.getFileSeparator() + screenshotName;

			if (screenshotPath.length() > 256) { // Max char limit for Windows
				// filenames
				screenshotName = reportSettings.getReportName()
						+ "_" + stepName.replaceAll("[^a-zA-Z0-9]+", "") + ".png";
				//screenshotPath = screenshotPath.substring(0, 256);
				screenshotPath = reportSettings.getReportPath() + Util.getFileSeparator() + SCREENSHOTS
						+ Util.getFileSeparator() + screenshotName;
			}

			if(stepStatus.equals(Status_CRAFT.FULLPAGESCREENSHOT)){
				takeFullPageScreenshot(screenshotPath);
			}else{
	...
	protected void takeFullPageScreenshot(String screenshotPath){
		System.out.println("this method from Report.java is overridden by method in SeleniumReport.java");
	}
